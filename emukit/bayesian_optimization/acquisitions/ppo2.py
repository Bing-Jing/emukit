...
class PPO(Acquisition):
    ...
    def __init__(self, model: Union[IModel, IDifferentiable] = None,load = False):
        ...
        self.funCurMax = 0
        self.funCurMin = float('inf')
        self.curFun = None
        self.ppoMax = 0
        self.ppoMin = float('inf')
        self.ep_r = 0
        self.buffer_ep = []
        self.s_ = None
    ...
    def save_model(self, steps):
        last_checkpoint = self.model_path + str(steps) + ".cptk"
...
if __name__ == "__main__":
    ...
    for ep in range(EP_MAX):
        ...
        if(ep%100 == 0):
            ...
            plt.plot(plotep, ppo.buffer_ep, "ro", markersize=20)
            # plt.show()
            plt.draw()
            fig1.savefig('traingin_reward3.png', dpi=100)
        ...
        l1, = plt.plot(bo.loop_state.X[num_data_points:], bo.loop_state.Y[num_data_points:], "ro", markersize=20, label="Observations")
        l0, = plt.plot(bo.loop_state.X[:num_data_points], bo.loop_state.Y[:num_data_points], "bo", markersize=20, label="init_sample")
        ...
        plt.plot(plotep, boPPOep_r, "ro", markersize=15,label="ppo")
        plt.annotate("final reward = {}".format(boPPOep_r[-1]),xy=(plotep[-1],boPPOep_r[-1]), xycoords="data")

        fig1 = plt.gcf()
        # plt.show()
        plt.draw()
        fig1.savefig('training_image3/ep_{}.png'.format(ep), dpi=100)
