...
if __name__ == "__main__":
    ...
    for ep in range(EP_MAX):
        ...
        for t in range(EP_LEN):    # in one episode
            ...
            s_ = np.concatenate((mu_,var_),axis=1)
            # boPPOep_r.append(ppo.ppoMax-env.maxVal)
            boPPOep_r.append(ppo.ppoMin-env.minVal)
        ...
        # plt.show()
        plt.draw()
        fig1.savefig('imageMin/ep_{}.png'.format(ep), dpi=100)
    ...
    # plt.show()
    plt.draw()
    fig1.savefig('traingin_rewardMin.png', dpi=100)
